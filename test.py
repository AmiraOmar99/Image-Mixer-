...
lib.dft.argtypes = [np.ctypeslib.ndpointer(ctypes.c_double, flags="C_CONTIGUOUS"), ctypes.c_int, np.ctypeslib.ndpointer(np.complex64, flags="C_CONTIGUOUS")]


lib.fft2.restype = None	
lib.fft2.argtypes = [np.ctypeslib.ndpointer(np.complex64, flags="C_CONTIGUOUS"), ctypes.c_int]
...
# for i in range(len(signal)):
    # print(signal[i]," >> " ,testdft[i])

signal =[1.5, 70.0, 81.0, 90.5, 0.0, 2.0, 15.7, 0.0]

# test = np.array(signal)
